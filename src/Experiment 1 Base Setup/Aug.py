...
#plt.show()
#img_list = [cv2.resize(img ,img_size)for _ in range(9)]
img_list = [img for _ in range(9)]

img_stack = np.concatenate((img_list), axis =-1)
...
class Aug_Hyperspectral_Data():
    ...
    def resize(self, array):
        return cv2.resize(array, self.size_tuple)
...
aug = Aug_Hyperspectral_Data(size_tuple = img_size) 
    
print(img_stack.shape)

augmented_array = aug(img_stack)
print("Original Array:", img_stack.shape)
print("Augmented Array:", augmented_array.shape)


plt.imshow(augmented_array[:, :, :3])
plt.axis('off')  # Turn off axis labels and ticks
plt.show()
