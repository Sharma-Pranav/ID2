...
class PickleDataset(Dataset):
    ...
    - shape (tuple): Desired shape of the images. Default is (512, 512).
    - transform (A.Compose): Augmentation transformation. Default is None.

    Methods:
    - __len__(): Returns the number of samples in the dataset.
    - __getitem__(idx): Retrieves and augments an image along with its label.

    Returns:
    - tuple: A tuple containing the augmented image and its label as torch.Tensor.
    """
    ...
    def __getitem__(self, idx):
        """
        Retrieves and augments an image along with its label.
...
def freeze_layers_except_first_m_last_n(model,m, n):
    """
    Freeze all layers except the first m and the last n layers of a PyTorch model.
    ...
    Returns:
    - nn.Module: Modified PyTorch model with specified layers unfrozen.
    """
    # Freeze all layers
...
def get_metric_dictionary(total_classes):
    """
    Create a dictionary of metrics for evaluating multiclass classification.

    Parameters:
    - total_classes (int): Total number of classes.

    Returns:
    - dict: Dictionary containing various metrics initialized for the given number of classes.
    """
    metric_dictionary = {
...
def train_test_loop(model, optimizer, criterion, epoch, trainloader, valloader, fold, total_classes):
    ...
    - dict: Dictionary containing various metrics evaluated on the validation set.
    """
...
class CombinedLoss(nn.Module):
    ...
    def __init__(self, alpha, beta, gamma):
        """
        Initialize CombinedLoss with alpha, beta, and gamma parameters.
...
def get_model(total_classes):
    """
    Initialize and return an EfficientNet-B3 model with a customized output layer.

    Parameters:
    - total_classes (int): The number of classes in the output layer.
